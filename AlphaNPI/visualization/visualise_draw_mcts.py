...
class MCTSvisualiser:
    ...
    def _print_node(self, node_idx, list_args, env_state, font_size=10):
        img_filename = f"img/{node_idx}.jpg"
        env_state.save(img_filename)

        self.file.write(self.indent + str(int(node_idx)) + ' [' + self._get_label_attr(list_args, font_size, ) + '\n')
        # self.file.write(self.indent + str(int(node_idx)) + f' imgnode[image="{img_filename}", ' + self._get_label_attr(list_args, font_size, ) + '\n')

    def _print_mcts_node(self, node):
        prior = node['prior']
        if prior is not None:
            prior = '%.2f'%(prior)
        beta = [round(num, 2) for num in node['Beta_Parameters'].tolist()]
        ...
        list_args=['prog : {}'.format(self.env.get_program_from_index(node['program_index'])),
                   'current: {}'.format(node["env_state"][1]),
                   'prior : {},  qvalue : {}'.format(prior, qvalue),
                   'beta : {}'.format(beta),
                   'depth : {}'.format(node['depth'])]
        
        env_state = self.env.get_state_img(node["env_state"])

        self._print_node(node['index'], list_args, env_state)

    def _print_mcts_edge(self, parent, child, color=None):
        label = f"{self.env.get_program_from_index(child['program_from_parent_index'])} <br/> {round(child['cval'] * 180/np.pi, 2) if child['cval'] is not None else ''}"
        self._print_edge(parent['index'], child['index'], label=label, color=color)
